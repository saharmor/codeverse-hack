...
from schemas import (
    ...
    Plan as PlanSchema, PlanCreate, PlanUpdate,
    PlanArtifact as PlanArtifactSchema, PlanArtifactCreate,
    ChatSession as ChatSessionSchema, ChatSessionCreate, ChatSessionUpdate
)
...
async def get_plan_versions(plan_id: str, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(Plan).where(
            Plan.repository_id == plan.repository_id,
            Plan.name == plan.name
