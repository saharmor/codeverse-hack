...
class Plan(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint('repository_id', 'name', 'version', name='uq_plans_repo_name_version'),
    )
    
    def __repr__(self):
        return f"<Plan(name='{self.name}', version={self.version}, status='{self.status.value}')>"
...
class PlanArtifact(BaseModel):
    ...
    def __repr__(self):
        return f"<PlanArtifact(type='{self.artifact_type.value}', plan_id='{self.plan_id}')>"