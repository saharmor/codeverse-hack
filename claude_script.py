...
from backend.services.claude_service import generate_plan as generate_plan_structured, ClaudeOutputType
...
async def run_iteration(
    ...
    async for out_type, chunk in generate_plan_structured(
        ...
        if out_type == ClaudeOutputType.PLAN:
...
def extract_plan_and_questions(output: str) -> tuple[Optional[str], Optional[str]]:
    """Extract the plan draft and clarifying questions from the output."""
    ...
    for line in lines:
        ...
        if "plan draft" in line_lower and line.startswith("#"):
...
async def _main() -> None:
    ...
    # Extract plan and questions from first iteration
    print("\n🔍 Extracting plan and questions from iteration 0...")
    extracted_plan, extracted_questions = extract_plan_and_questions(iteration_0_output)
